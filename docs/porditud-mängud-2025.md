```

                          __  _  _  _
            '     /       _/ / )/ )/ )
           / / / /_/ / /   // // // /
          / (_( /(_ (_( (_/(_/(_/(_/
         /
      __/   ** PORDITUD MÄNGUD 2025 **

```  
  
JUKU E5104 peale kirjutatakse mänge ka aastal 2025,
aga peamiselt neid siiski porditakse. Lisaks tuntud
ja tundmatutele koopiatele leiad MMXXV kettalt
unarvara uusversioone 2024-2025.

``ARKANOID.COM`` - Arkanoid 1.0 (Brick '90/'25)  
``BINLAND .COM`` - Binary Land (Hudson '83/'24)  
``BOMB    .COM`` - Bomber Man (Hudson '83/'24)  
``BOULDER .COM`` - Boulder Dash (First Star '84/'25)  
``CBALL   .COM`` - Cannon Ball (Hudson '83/'25)  
``CLRBALL .COM`` - Color Ball (Hudson '84/'25)  
``DIGGER  .COM`` - Tuntud mängu ainetel (K.Tomingas '91)  
``DTANK   .COM`` - Driller Tanks (Hudson '83/'24)  
``FIRE    .COM`` - Fire Rescue (Hudson '84/'25)  
``HITCH   .COM`` - Pöidlaküüdi reisijuht (IC '85/'24)  
``INDIAN  .COM`` - Indian no Bouken (Hudson '83/'25)  
``LAD     .   ­`` - Ladderi graafika fonditabelina  
``LADDER2 .COM`` - Platvormimäng Ladder 2 (Yahoo '83)  
``LAOLEO  .COM`` - Laoleo (=Sokoban, Kompuuter '91 H)  
``LODEGEN .COM`` - Zoo leveliredaktor (V.Zverkov '90)  
``LR      .COM`` - Zoo demoversioon\* (üks level)  
``MADUOK  .COM`` - Klassikaline ussimäng (=Snake)  
``NINJA   .COM`` - Ninjya Kage (Hudson '84/'24)  
``PUSHER  .COM`` - Nügiline\* (=Sokoban, A.Mett '91)  
``PUSHERPK.COM`` - Nügilise levelite pakkija  
``PUTUP   .COM`` - Put Up (Y.Ago/MSX Magazine '87/'25)  
``READ    .ME ­`` - Tekst, mida praegu loed  
``ROBBO   .COM`` - Robbo (LK Avalon '89/'25)  
``STOP    .COM`` - Stop the Train (Hudson '83/'24)  
``ZOKOBAN .COM`` - Zokoban (Errorsoft/Syntax '91)  
``ZOO1    .COM`` - Zoo algversioon\* (9 levelit, VSW '91)  
``TANK    .COM`` - Tank Battalion (Namco '84/'25)  
``TET2    .COM`` - Tetris 1.1 (A.Mett '90)  
``TETRIS  .COM`` - Graafiline Tetris (T.Ainsaar '90)  
``TKUK    .COM`` - Tähtede kukkumine (Tramm '90/'25)  
``TOTDEM  .COM`` - Totruse demo (Joga/Mamsoft '90/'25)  
``USSIKE  .COM`` - Zmejka (A.Alekseev '24/'25)  
``WARP    .COM`` - Warp & Warp (Namco '84/'24)  
``XONIX   .COM`` - Xonix\* (=Qix, E.Jürviste/EKTA '87)  

Enamiku värsketest portidest on teinud vanameister Pehka1985, kui pole märgitud teisiti.
Totruse ja Pöidlaküüdi reisijuhi portis Tramm. Viimane ja Ladder vajavad 80x24 kuvalaotust,
mille aitab sisse lülitada `MODX`. Ladderi jaoks on eraldi font. 

Ketta saab alla laadida [siit](https://elektroonikamuuseum.ee/failid/juku/tarkvara/J3KGAME2.JUK) ja esialgu kattub see veebiemulaatori `A:` kettaga. Põnevaid seiklusi retroraalimise imemaailmas!

## Vaata ka

* [Ketas veebiemulaatoris](https://infoaed.ee/juku/) (2025 lõpu seisuga)
* [Ketta failide täisnimekiri](tarkvara-kataloog.md#j3kgame2)
* [Mängude ketas 2024](mängude-ketas-2024.md)
* [Lühijuhend EKDOSis orienteerumiseks](juku-käsud.md)
* [Emulaatori käivitamine töölaual](mame-käivitamine.md)
